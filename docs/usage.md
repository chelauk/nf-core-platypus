# nf-core/platypus: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/platypus/usage](https://nf-co.re/platypus/usage)
...
```console
nextflow run nf-core/platypus --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/platypus
```
...
First, go to the [nf-core/platypus releases page](https://github.com/nf-core/platypus/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
